...
class CMB2_Tabs {
	...
	public function __construct() {
		add_action( 'cmb2_render_tabs', [ $this, 'render' ], 10, 5 );
		add_filter( 'cmb2_sanitize_tabs', [ $this, 'save' ], 10, 4 );
	}
	...
	public function render( \CMB2_Field $field_object, $escaped_value, $object_id, $object_type, \CMB2_Types $field_type_object ) {
		...
		] : $field_type_object->parse_args( $field_object->data_args(), 'tabs', [
			'class' => "ui-tabs dtheme-cmb2-tabs $layout",
		] );

		// Render field
		echo sprintf( '<div %s>%s</div>', $field_type_object->concat_attrs( $default_data, [
			'value',
			'name',
			'type',
		] ), $this->get_tabs() );
        ...
        <ul class="ui-tabs-nav">
			<?php foreach ( $this->setting['tabs'] as $key => $tab ): ?>
                <li class="<?php if ( $key == 0 ) {
					echo ' ui-tabs-active ';
				} ?>"><a class="ui-tabs-anchor" href="#<?php echo $tab['id']; ?>"><?php echo $tab['title']; ?></a></li>
			<?php endforeach; ?>
        </ul>

		<?php foreach ( $this->setting['tabs'] as $key => $tab ): ?>
            <div class="ui-tabs-panel<?php if ( $key == 0 ) {
				echo ' ui-tabs-active';
			} ?>" id="<?php echo $tab['id']; ?>">
				<?php
	...
	public function render_fields( $args, $fields, $object_id ) {
		// Set options to cmb2
		$setting_fields = array_merge( $args, [ 'fields' => $fields ] );
	...
	public static function save( $override_value, $value, $post_id, $data ) {
		foreach ( $data['tabs']['tabs'] as $tab ) {
			$setting_fields = array_merge( $data['tabs']['config'], [ 'fields' => $tab['fields'] ] );
